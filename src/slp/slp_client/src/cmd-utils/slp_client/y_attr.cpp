...
{
       0,    83,    93,   105,   112,   119,   127,   139,   146,   153,
     156,   159,   162,   176
};
#endif
...
#line 162 "attr.y"
{
				...
				if( *(yyvsp[0]._s) == '\\' && ((*(yyvsp[0]._s + 1) == 'f') || (*(yyvsp[0]._s + 1) == 'F')) &&  ((*(yyvsp[0]._s + 2) == 'f') || (*(yyvsp[0]._s + 2) == 'F'))) {
				       ...
				       yyval._atl = lslpAllocAttr(NULL, string_type, yyvsp[0]._s, (int16)(strlen(yyvsp[0]._s) + 1));
				     }
                                  }
			     else {
...
void lslpFreeAttr(lslpAttrList *attr)
{
  ...
  if (attr->type == string_type && attr->val.stringVal != NULL)
