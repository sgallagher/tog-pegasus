...
ifeq ($(COMPILER),gnu)
  ifneq ($(PEGASUS_PLATFORM),DARWIN_PPC_GNU)
   ifdef PEGASUS_USE_RELEASE_DIRS
      ...
      LINK_ARGUMENTS = -Wl,-hlib$(LIBRARY)$(LIB_SUFFIX)  -Xlinker -rpath -Xlinker $(PEGASUS_DEST_LIB_DIR) $(EXTRA_LINK_FLAGS)
   else
      LINK_COMMAND = g++ -shared
      LINK_ARGUMENTS = -Wl,-hlib$(LIBRARY)$(LIB_SUFFIX)  -Xlinker -rpath -Xlinker $(LIB_DIR) $(EXTRA_LINK_FLAGS)
   endif
  else
